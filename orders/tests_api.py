...
class OrderAPITestCase(TestCase):
    def setUp(self):
        ...
        self.order2 = Order.objects.create(user=self.user, created_at=now - timedelta(days=1))

        self.item1 = OrderItem1.objects.create(order=self.order1, price=Decimal("100.00"), created_at=now)
        self.item2 = OrderItem2.objects.create(
            order=self.order1, placement_price=Decimal("50.00"), article_price=Decimal("30.00"), created_at=now
        )
...
class ReportAPITestCase(TestCase):
    def setUp(self):
        ...
        self.order1 = Order.objects.create(user=self.user1, created_at=base_date)
        self.order2 = Order.objects.create(user=self.user2, created_at=base_date + timedelta(days=1))

        OrderItem1.objects.create(order=self.order1, price=Decimal("100.00"), created_at=base_date)
    ...
    def test_daily_report(self):
        ...
        response = self.client.get(url, {"start_date": "2025-01-10", "end_date": "2025-01-13"})
    ...
    def test_weekly_report(self):
        ...
        response = self.client.get(url, {"start_date": "2025-01-05", "end_date": "2025-01-20"})
    ...
    def test_monthly_report(self):
        ...
        response = self.client.get(url, {"start_date": "2025-01-01", "end_date": "2025-03-01"})
    ...
    def test_report_data_structure(self):
        url = reverse("report-daily")
        response = self.client.get(url, {"start_date": "2025-01-10", "end_date": "2025-01-11"})
