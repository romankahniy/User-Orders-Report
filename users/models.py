...
class UserQuerySet(models.QuerySet):
    def with_statistics(self):
        return self.annotate(
            ...
            orderitem1_total=Coalesce(
                Sum('orders__items1__price', distinct=True),
                0,
                output_field=DecimalField()
            ),
            orderitem2_placement_total=Coalesce(
                Sum('orders__items2__placement_price', distinct=True),
                0,
                output_field=DecimalField()
            ),
            orderitem2_article_total=Coalesce(
                Sum('orders__items2__article_price', distinct=True),
                0,
                output_field=DecimalField()
            ),
        ).annotate(
